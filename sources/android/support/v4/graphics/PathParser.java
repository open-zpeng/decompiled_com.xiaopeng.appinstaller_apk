    ...
    public static class PathDataNode {
        ...
        private static void drawArc(Path p, float x0, float y0, float x1, float y1, float a2, float b2, float theta, boolean isMoreThanHalf, boolean isPositiveArc) {
            ...
            if (disc < 0.0d) {
                ...
                drawArc(p, x0, y0, x1, y1, a2 * adjust, b2 * adjust, theta, isMoreThanHalf, isPositiveArc);
                return;
            }
            ...
            double cy2 = cy * b2;
            double tcx = a2;
            arcToBezier(p, (cx2 * cosTheta) - (cy2 * sinTheta), (cx2 * sinTheta) + (cy2 * cosTheta), tcx, b2, x0, y0, thetaD, eta0, sweep);
        }
        ...
        private static void arcToBezier(Path p, double cx, double cy, double a2, double b2, double e1x, double e1y, double theta, double start, double sweep) {
            ...
            double ep1x = (((-eta1) * cosTheta) * sinEta1) - ((b2 * sinTheta) * cosEta1);
            double ep1x2 = -eta1;
            ...
            while (true) {
                ...
                double anglePerSegment3 = (cx + ((eta1 * cosTheta) * cosEta2)) - ((b2 * sinTheta) * sinEta2);
                double e2x = cy + (eta1 * sinTheta * cosEta2) + (b2 * cosTheta * sinEta2);
                double e2y = -eta1;
                double ep2x = ((e2y * cosTheta) * sinEta2) - ((b2 * sinTheta) * cosEta2);
                double ep2y = ((-eta1) * sinTheta * sinEta2) + (b2 * cosTheta * cosEta2);
