...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void setMeasuredDimension(Rect childrenBounds, int wSpec, int hSpec) {
        ...
        if (this.mOrientation == 1) {
            ...
            int usedHeight2 = chooseSize(wSpec, (this.mSizePerSpan * this.mSpanCount) + horizontalPadding, getMinimumWidth());
            height = usedHeight2;
            width = height2;
        } else {
    ...
    private void fixStartGap(RecyclerView.Recycler recycler, RecyclerView.State state, boolean canOffsetChildren) {
        int gap;
        int minStartLine = getMinStart(Preference.DEFAULT_ORDER);
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        if (this.mLayoutState.mInfinite) {
            if (layoutState.mLayoutDirection == 1) {
                targetLine = Preference.DEFAULT_ORDER;
            } else {
        ...
        int targetLine4 = layoutState.mLayoutDirection;
        updateAllRemainingSpans(targetLine4, targetLine3);
        if (this.mShouldReverseLayout) {
            startAfterPadding = this.mPrimaryOrientation.getEndAfterPadding();
        } else {
            startAfterPadding = this.mPrimaryOrientation.getStartAfterPadding();
        }
        int defaultNewViewLine2 = startAfterPadding;
        int targetLine5 = 0;
        while (true) {
            added = targetLine5;
            boolean added2 = layoutState.hasMore(state);
            if (!added2) {
                i = r14;
                break;
            } else if (!this.mLayoutState.mInfinite && this.mRemainingSpans.isEmpty()) {
                i = r14;
                break;
            } else {
                View view2 = layoutState.next(recycler);
                ...
                boolean assignSpan = spanIndex == -1 ? r15 : r14;
                if (assignSpan) {
                    currentSpan = lp2.mFullSpan ? this.mSpans[r14] : getNextSpan(layoutState);
                ...
                if (layoutState.mLayoutDirection == r15) {
                    addView(view2);
                } else {
                    addView(view2, r14);
                }
                ...
                if (layoutState.mLayoutDirection == r15) {
                    int start2 = lp2.mFullSpan ? getMaxEnd(defaultNewViewLine2) : currentSpan3.getEndLine(defaultNewViewLine2);
                    int end2 = this.mPrimaryOrientation.getDecoratedMeasurement(view2) + start2;
                    ...
                    int end3 = lp2.mFullSpan ? getMinStart(defaultNewViewLine2) : currentSpan3.getStartLine(defaultNewViewLine2);
                    int start3 = end3 - this.mPrimaryOrientation.getDecoratedMeasurement(view2);
                    if (assignSpan && lp2.mFullSpan) {
                        LazySpanLookup.FullSpanItem fullSpanItem2 = createFullSpanItemFromStart(end3);
                        fullSpanItem2.mGapDir = r15;
                ...
                if (lp2.mFullSpan && layoutState.mItemDirection == -1) {
                    if (assignSpan) {
                        this.mLaidOutInvalidFullSpan = r15;
                    } else {
                        if (layoutState.mLayoutDirection == r15) {
                            i2 = (areAllEndsEqual() ? 1 : 0) ^ r15;
                        } else {
                            boolean hasInvalidGap = areAllStartsEqual();
                            i2 = (hasInvalidGap ? 1 : 0) ^ r15;
                        }
                        if (i2 != 0) {
                            LazySpanLookup.FullSpanItem fullSpanItem3 = this.mLazySpanLookup.getFullSpanItem(position);
                            if (fullSpanItem3 != null) {
                                fullSpanItem3.mHasUnwantedGapAfter = r15;
                            }
                            this.mLaidOutInvalidFullSpan = r15;
                        }
                    }
                }
                attachViewToSpans(view2, lp2, layoutState);
                if (isLayoutRTL() && this.mOrientation == r15) {
                    int otherEnd2 = lp2.mFullSpan ? this.mSecondaryOrientation.getEndAfterPadding() : this.mSecondaryOrientation.getEndAfterPadding() - (((this.mSpanCount - r15) - currentSpan3.mIndex) * this.mSizePerSpan);
                    otherEnd = otherEnd2;
                    otherStart = otherEnd2 - this.mSecondaryOrientation.getDecoratedMeasurement(view2);
                } else {
                    int otherStart2 = lp2.mFullSpan ? this.mSecondaryOrientation.getStartAfterPadding() : (currentSpan3.mIndex * this.mSizePerSpan) + this.mSecondaryOrientation.getStartAfterPadding();
                    otherStart = otherStart2;
                    otherEnd = this.mSecondaryOrientation.getDecoratedMeasurement(view2) + otherStart2;
                }
                if (this.mOrientation == r15) {
                    ...
                    view = view2;
                    defaultNewViewLine = defaultNewViewLine2;
                    int defaultNewViewLine3 = end;
                    targetLine2 = targetLine3;
                    int targetLine6 = otherEnd;
                    layoutDecoratedWithMargins(view2, start, otherStart, defaultNewViewLine3, targetLine6);
                }
                if (lp.mFullSpan) {
        ...
        int maxEnd = diff;
        return maxEnd > 0 ? Math.min(layoutState.mAvailable, maxEnd) : i;
    }
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state, RecyclerView.LayoutManager.LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        for (int itemPrefetchCount2 = 0; itemPrefetchCount2 < this.mSpanCount; itemPrefetchCount2++) {
            if (this.mLayoutState.mItemDirection == -1) {
                distance = this.mLayoutState.mStartLine - this.mSpans[itemPrefetchCount2].getStartLine(this.mLayoutState.mStartLine);
            } else {
                distance = this.mSpans[itemPrefetchCount2].getEndLine(this.mLayoutState.mEndLine) - this.mLayoutState.mEndLine;
            }
            if (distance >= 0) {
        ...
        while (true) {
            int i2 = i;
            if (i2 < itemPrefetchCount && this.mLayoutState.hasMore(state)) {
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (preferLastSpan(layoutDir)) {
            for (int i2 = this.mSpanCount - 1; i2 >= 0; i2--) {
                View view2 = this.mSpans[i2].getFocusableViewAfter(referenceChildPosition, layoutDir);
            ...
            for (int i3 = 0; i3 < this.mSpanCount; i3++) {
                View view3 = this.mSpans[i3].getFocusableViewAfter(referenceChildPosition, layoutDir);
