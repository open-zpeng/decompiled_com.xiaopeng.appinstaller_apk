...
public final class GapWorker implements Runnable {
    ...
    private void buildTaskList() {
        ...
        int j = 0;
        for (int totalTaskIndex = 0; totalTaskIndex < viewCount; totalTaskIndex++) {
            RecyclerView view2 = this.mRecyclerViews.get(totalTaskIndex);
            if (view2.getWindowVisibility() == 0) {
                ...
                int totalTaskIndex2 = j;
                for (int totalTaskIndex3 = 0; totalTaskIndex3 < prefetchRegistry.mCount * 2; totalTaskIndex3 += 2) {
                    ...
                    int distanceToItem = prefetchRegistry.mPrefetchArray[totalTaskIndex3 + 1];
                    task.immediate = distanceToItem <= viewVelocity;
                    task.viewVelocity = viewVelocity;
                    task.distanceToItem = distanceToItem;
                    task.view = view2;
                    task.position = prefetchRegistry.mPrefetchArray[totalTaskIndex3];
                    totalTaskIndex2++;
                }
                j = totalTaskIndex2;
            }
