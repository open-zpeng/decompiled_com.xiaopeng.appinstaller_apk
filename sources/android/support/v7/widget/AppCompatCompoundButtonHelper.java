...
public class AppCompatCompoundButtonHelper {
    ...
    public void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        int resourceId;
        TypedArray a2 = this.mView.getContext().obtainStyledAttributes(attrs, R.styleable.CompoundButton, defStyleAttr, 0);
        try {
            if (a2.hasValue(R.styleable.CompoundButton_android_button) && (resourceId = a2.getResourceId(R.styleable.CompoundButton_android_button, 0)) != 0) {
                this.mView.setButtonDrawable(AppCompatResources.getDrawable(this.mView.getContext(), resourceId));
            }
            if (a2.hasValue(R.styleable.CompoundButton_buttonTint)) {
                CompoundButtonCompat.setButtonTintList(this.mView, a2.getColorStateList(R.styleable.CompoundButton_buttonTint));
            }
            if (a2.hasValue(R.styleable.CompoundButton_buttonTintMode)) {
                CompoundButtonCompat.setButtonTintMode(this.mView, DrawableUtils.parseTintMode(a2.getInt(R.styleable.CompoundButton_buttonTintMode, -1), null));
            }
        } finally {
            a2.recycle();
        }
