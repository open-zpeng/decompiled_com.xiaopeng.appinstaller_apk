...
public class PagedListView extends FrameLayout {
    ...
    public PagedListView(Context context) {
        ...
        this.mRecyclerViewOnScrollListener = new RecyclerView.OnScrollListener() { // from class: androidx.car.widget.PagedListView.2
        ...
        this.mPaginationRunnable = new Runnable() { // from class: androidx.car.widget.PagedListView.3
        ...
        this.mUpdatePaginationRunnable = new Runnable() { // from class: androidx.car.widget.-$$Lambda$PagedListView$Vf9wSInQhS-xhuyshQUf131oISE
            @Override // java.lang.Runnable
            public final void run() {
                PagedListView.this.updatePaginationButtons(true);
            }
        };
        init(context, null);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.PagedListView, R.attr.pagedListViewStyle, 0);
        ...
        if (a2.getBoolean(R.styleable.PagedListView_verticallyCenterListContent, false)) {
            this.mRecyclerView.getLayoutParams().height = -2;
        }
        int defaultGutterSize = getResources().getDimensionPixelSize(R.dimen.car_margin);
        this.mGutterSize = a2.getDimensionPixelSize(R.styleable.PagedListView_gutterSize, defaultGutterSize);
        if (a2.hasValue(R.styleable.PagedListView_gutter)) {
            int gutter = a2.getInt(R.styleable.PagedListView_gutter, 3);
            setGutter(gutter);
        } else if (a2.hasValue(R.styleable.PagedListView_offsetScrollBar)) {
            boolean offsetScrollBar = a2.getBoolean(R.styleable.PagedListView_offsetScrollBar, false);
            if (offsetScrollBar) {
                setGutter(1);
            }
        } else {
        ...
        if (a2.getBoolean(R.styleable.PagedListView_showPagedListViewDivider, true)) {
            int dividerStartMargin = a2.getDimensionPixelSize(R.styleable.PagedListView_dividerStartMargin, 0);
            int dividerEndMargin = a2.getDimensionPixelSize(R.styleable.PagedListView_dividerEndMargin, 0);
            int dividerStartId = a2.getResourceId(R.styleable.PagedListView_alignDividerStartTo, -1);
            int dividerEndId = a2.getResourceId(R.styleable.PagedListView_alignDividerEndTo, -1);
            int listDividerColor = a2.getResourceId(R.styleable.PagedListView_listDividerColor, R.color.car_list_divider);
            i = -1;
        ...
        int itemSpacing = a2.getDimensionPixelSize(R.styleable.PagedListView_itemSpacing, 0);
        if (itemSpacing > 0) {
            this.mRecyclerView.addItemDecoration(new ItemSpacingDecoration(itemSpacing));
        }
        int listContentTopMargin = a2.getDimensionPixelSize(R.styleable.PagedListView_listContentTopOffset, 0);
        if (listContentTopMargin > 0) {
            this.mRecyclerView.addItemDecoration(new TopOffsetDecoration(listContentTopMargin));
        }
        setFocusable(false);
        this.mScrollBarEnabled = a2.getBoolean(R.styleable.PagedListView_scrollBarEnabled, true);
        this.mScrollBarView = (PagedScrollBarView) findViewById(R.id.paged_scroll_view);
        this.mScrollBarView.setPaginationListener(new PagedScrollBarView.PaginationListener() { // from class: androidx.car.widget.PagedListView.1
        ...
        Drawable upButtonIcon = a2.getDrawable(R.styleable.PagedListView_upButtonIcon);
        if (upButtonIcon != null) {
            setUpButtonIcon(upButtonIcon);
        }
        Drawable downButtonIcon = a2.getDrawable(R.styleable.PagedListView_downButtonIcon);
        if (downButtonIcon != null) {
            setDownButtonIcon(downButtonIcon);
        }
        int scrollBarColor = a2.getResourceId(R.styleable.PagedListView_scrollBarColor, i);
        if (scrollBarColor != i) {
            setScrollbarColor(scrollBarColor);
        }
        this.mScrollBarView.setVisibility(this.mScrollBarEnabled ? 0 : 8);
        if (this.mScrollBarEnabled) {
            int topMargin = a2.getDimensionPixelSize(R.styleable.PagedListView_scrollBarTopMargin, ((ViewGroup.MarginLayoutParams) this.mScrollBarView.getLayoutParams()).topMargin);
        ...
        if (a2.hasValue(R.styleable.PagedListView_scrollBarContainerWidth)) {
            int carMargin = getResources().getDimensionPixelSize(R.dimen.car_margin);
            int scrollBarContainerWidth = a2.getDimensionPixelSize(R.styleable.PagedListView_scrollBarContainerWidth, carMargin);
            setScrollBarContainerWidth(scrollBarContainerWidth);
        }
        int carMargin2 = R.styleable.PagedListView_dayNightStyle;
        if (a2.hasValue(carMargin2)) {
            int dayNightStyle = a2.getInt(R.styleable.PagedListView_dayNightStyle, 0);
    ...
    public void setGutter(int gutter) {
        ...
        this.mRecyclerView.setClipToPadding((startMargin == 0 && endMargin == 0) ? false : false);
    }
    ...
    public void snapToPosition(final int position) {
        ...
        if (startPosition != position) {
            post(new Runnable() { // from class: androidx.car.widget.-$$Lambda$PagedListView$yLFTxQZlMxlLNIv62uHqg1XZmao
                @Override // java.lang.Runnable
                public final void run() {
                    PagedListView.this.scrollToPosition(position);
                }
            });
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: classes.dex */
    public static class TopOffsetDecoration extends RecyclerView.ItemDecoration {
