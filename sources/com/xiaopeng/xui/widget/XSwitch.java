...
public class XSwitch extends XCompoundButton implements ValueAnimator.AnimatorUpdateListener {
    ...
    public XSwitch(Context context, AttributeSet attributeSet) {
        ...
        float f2 = this.OUT_STROKE_RADIUS * 2.0f;
        this.OUT_STROKE_BOTTOM = f + f2;
        this.OUT_CIRCLE_BOTTOM = this.OUT_STROKE_BOTTOM + this.DIMEN_2;
        this.OUT_STROKE_RIGHT = this.OUT_STROKE_LEFT + f2;
        ...
        float f6 = this.INNER_CIRCLE_RADIUS * 2.0f;
        this.INNER_BOTTOM = f5 + f6;
        this.INNER_CIRCLE_RIGHT = this.INNER_LEFT + f6;
    ...
    private void changeParams() {
        ...
        if (isEnabled()) {
            if (isPressed()) {
                ...
                i4 = this.mBgEnableOffPressedColorEnd;
            } else {
                ...
                i4 = this.mBgEnableOffNormalColorEnd;
            }
            this.mBgOffColorEnd = i4;
            ...
            i3 = this.mVerticalEnableOnColor;
        } else {
            ...
            i3 = this.mVerticalDisableOnColor;
        }
        this.mVerticalOnColor = i3;
    ...
    private void resetAnimValue() {
        this.mCurrentAnimValue = isChecked() ? ANIMATOR_DURATION : 0;
    }
    ...
    private void updateBgFraction() {
        int i;
        int i2;
        int i3 = this.mCurrentAnimValue;
        if (i3 <= 200) {
            i = this.mBgOffColorStart;
            i2 = this.mBgOffColorEnd;
        } else if (i3 <= 500) {
            i = gradientColor(this.mBgOffColorStart, this.mBgOnColorStart, this.transPercent);
            i2 = gradientColor(this.mBgOffColorEnd, this.mBgOnColorEnd, this.transPercent);
        } else {
            i = this.mBgOnColorStart;
            i2 = this.mBgOnColorEnd;
        }
        updateBgShader(i, i2);
    }
    ...
    private void updateCircleMoveFraction() {
        ...
        this.mTranslateX = i <= 200 ? 0.0f : i <= 500 ? this.MOVE_X * this.transPercent : this.MOVE_X;
    }
    ...
    private void updateCircleOutFraction() {
        ...
        if (i2 <= 200) {
            ...
            f = (this.mBlurAlphaDst * this.mCurrentAnimValue) / 200.0f;
            changeColorAlpha = changeColorAlpha(f, this.mCircleOutOffColorStart);
            i = this.mCircleOutOffColorEnd;
        } else if (i2 <= 500) {
            ...
            f = f4 - ((((this.mCurrentAnimValue - 300) - 200) * f4) / 200.0f);
            changeColorAlpha = changeColorAlpha(f, this.mCircleOutOnColorStart);
            i = this.mCircleOutOnColorEnd;
        }
        updateCircleOutLightShader(changeColorAlpha, changeColorAlpha(f, i));
    }
    ...
    private void updateVerticalFraction() {
        ...
        if (i <= 200) {
            ...
            f = (this.mBlurAlphaDst * i) / 200.0f;
        } else if (i <= 500) {
            this.mVerticalColor = gradientColor(this.mVerticalOffColor, this.mVerticalOnColor, this.transPercent);
            float f2 = ((this.mCurrentAnimValue - 200) - 120) / 180.0f;
            if (f2 >= 0.0f) {
                float f3 = this.VER_LEFT;
                this.mVerticalLeft = f3 + ((this.VER_RIGHT - f3) * f2);
                float f4 = this.CENTER_Y;
                this.mVerticalTop = f4 - ((f4 - this.VER_TOP_RIGHT) * f2);
                float f5 = this.VER_WIDTH_LEFT;
                this.mVerticalWidth = f5 - ((f5 - this.VER_WIDTH_RIGHT) * f2);
                this.mVerticalHeight = this.VER_HEIGHT_RIGHT * f2;
            }
            changeColorAlpha = changeColorAlpha(this.mBlurAlphaDst, this.mVerticalColor);
            this.mVerticalLightColor = changeColorAlpha;
        } else {
            ...
            float f6 = this.mBlurAlphaDst;
            f = f6 - ((((i - 300) - 200) * f6) / 200.0f);
        }
        changeColorAlpha = changeColorAlpha(f, this.mVerticalColor);
        this.mVerticalLightColor = changeColorAlpha;
    }
    ...
    public void onConfigurationChanged(Configuration configuration) {
        ...
        String str = TAG;
        Log.d(str, "onConfigurationChanged：" + XThemeManager.isThemeChanged(configuration));
    ...
    public void setChecked(boolean z) {
        ...
        if (onInterceptListener == null || !onInterceptListener.onInterceptCheck(this, z)) {
            ...
            if (!isAttachedToWindow() || getWidth() == 0) {
                ...
                stopAnimation();
                runAnimator(z ? ANIMATOR_DURATION : 0);
            }
        }
    }
